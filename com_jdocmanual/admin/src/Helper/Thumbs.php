...
class Thumbs
{
    ...
    public function create($img, $options, $srcSets)
    {
        if ($this->driver === 'imagick') {
            return $this->createImagic($img, $options, $srcSets);
        } elseif ($this->driver === 'gd') {
            return $this->createGd($img, $options, $srcSets);
        } else {
            return;
        }
    }
        ...
        for ($i = 0, $l = count($options->validSizes); $i < $l; $i++) {
            if ($options->scaleUp || ($img->width >= (int) $options->validSizes[$i])) {
                ...
                if ($options->enableWEBP && \imagewebp($destImage, JPATH_ROOT . '/' . $fileSrc . '.webp', $options->qualityWEBP)) {
                    ...
                    if (!isset($srcSets->webp)) {
                        $srcSets->webp = (object) ['srcset' => []];
                    }
    ...
    private function createImagic($img, $options, $srcSets)
    {
        for ($i = 0, $l = count($options->validSizes); $i < $l; $i++) {
            if ($options->scaleUp || ($img->width >= (int) $options->validSizes[$i])) {
                $fileSrc = 'jdmimages/manuals/' . $img->dirname . '/' . $img->filename . $options->separator . trim($options->validSizes[$i]);
                $image = new \Imagick();
    ...
    {
        if (extension_loaded($preferedDriver)) {
